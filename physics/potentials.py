...
class PedPedPotential(object):
    ...
    def r_ab(state):
        ...
        r_ab := r_a − r_b.
        """
        return states.vec_diff(state[:, :2])

    def __call__(self, state):
        speeds = states.speeds(state)
        return self.value_r_ab(self.r_ab(state), speeds, states.desired_directions(state))

    def grad_r_ab(self, state, delta=1e-3):
        """Compute gradient wrt r_ab using finite difference differentiation."""
        r_ab = self.r_ab(state)
        speeds = states.speeds(state)
        desired_directions = states.desired_directions(state)
